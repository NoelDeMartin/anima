<template>
  <GuestLayout>
    ...
    <Form :form @submit="submit" class="flex flex-col items-center max-w-75 w-full gap-2">
      <Input
        name="email"
        type="email"
        :label="$t('home.email')"
      ...
      <Button submit class="w-full mt-1" :disabled="loading">
        {{ $t('home.register') }}
      </Button>
      <Button variant="link" route="home" class="w-full text-center lowercase">
        {{ $t('ui.cancel') }}
      </Button>
    </Form>
  </GuestLayout>
...
async function submit() {
  if (form.confirmPassword !== form.password) {
    errorMessage.value = translate('home.registerPasswordsDontMatch');
  }
